...
t = Toplevel(b)
b.geometry('+%d+%d' % (11, 0))
b.geometry("1500x1500")
b.configure(background='black')

photo_button = Button(b, text='open', command=open_photo,
                      height=4, width=40, state=NORMAL,
                      font=('calibri', 20, 'bold'),
                      borderwidth='4', background='green')
# photo_button.grid(row=5,column=0)

photo_k = False

snap_button = Button(b, text='Photo', command=take_photo,
                     height=2, width=25, state=NORMAL,
                     font=('calibri', 20, 'bold'), fg='yellow',
                     borderwidth='4', background='azure4')

snap_button.grid(row=2, column=0)
date_var = StringVar(b)
flag = False
folder = 'a'

folder_varc = StringVar(b)

# hem = Toplevel(b)

# hem.geometry('+%d+%d' % (10, 350))
# hem.geometry('400x100')

l_hb = Label(b, height=2, width=20, text="Hemoglobin",
             fg='White', font=('calibri', 30, 'bold'), background='black')

# hem.attributes("-topmost", True)

# l_hb.config(font =("Courier bold", 30),background='black')

l_hb.grid(row=0, column=0, ipady=100)
# hem.configure(background='black')
t.configure(background='black')

predict_button = Button(b, text='Predict',
                        command=predict_hb,
                        height=2, width=25, state=NORMAL,
...
startbutton = Button(b, text='SFDI', command=start,
                     height=2, width=25, state=NORMAL,
                     font=('calibri', 20, 'bold'), fg='white',
                     borderwidth='4', background='azure4')
...
t.geometry('%dx%d+%d+%d' % (screen_width, screen_height, x, y))
t.state('zoomed')
...
R3 = Radiobutton(b, text="630+550nm", variable=var, value=10, command=sel)

var.set(10)
# label = Label(b)
# label.config(text = 'ms')
# label.grid(row=2,column=1)

folder = 'a'

folder_var = StringVar(b)

date_var = StringVar(b)
# date_var=str(datetime.date(datetime.now()))

e1 = Entry(b, textvariable=folder_var, width=10)
counter = np.random.randint(10000000)
e1.insert(0, str(counter))
# e1.grid(row=0, column=1)


e2 = Entry(b, textvariable=date_var, fg='red', width=5)

e2.insert(0, str(datetime.date(datetime.now())))
...
b.bind('<Escape>', lambda x: b.destroy())
...
# l.config(image=ImageTk.PhotoImage(data_freq_red.T[0]['000']))
# white_im=ImageTk.PhotoImage(data_freq_red.T[0]['000'])
